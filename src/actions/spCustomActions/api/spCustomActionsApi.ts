import CustomActionItem from "../components/spCustomActionsItem";
...
export default class SpCustomActionsApi extends ApiBase {
    ...
    public async getCustomActions(caType: CustomActionLocationType): Promise<ICustomAction[]> {
        const webUrl: string = await this.getWebUrl();

        const reqUrl = `${webUrl}/_api/${caType}${constants.CUSTOM_ACTION_REST_REQUEST_URL}`;
    ...
    public deleteCustomAction(caObj: ICustomAction, caType: CustomActionLocationType): Promise<ICustomAction> {
        return new Promise((resolve, reject) => {
            const caGuid: SP.Guid = new SP.Guid(caObj.id);
            const ctx: SP.ClientContext = SP.ClientContext.get_current();
            const ca: SP.UserCustomAction = (caType === CustomActionItem.Web)
            ...
            ctx.executeQueryAsync((sender: any, err: any) => {
                resolve(caObj);
            }, this.getErrorResolver(constants.ERROR_MESSAGE_DELETING_CUSTOM_ACTION));
        });
    }

    public createCustomAction(ca: ICustomAction, caType: CustomActionLocationType): Promise<ICustomAction> {
        return this.setCustomAction(ca, caType, true);
    }

    public updateCustomAction(ca: ICustomAction, caType: CustomActionLocationType): Promise<ICustomAction> {
        return this.setCustomAction(ca, caType, false);
    }

    private setCustomAction(caObj: ICustomAction, caType: CustomActionLocationType, isNewCa: boolean): Promise<ICustomAction> {
        return new Promise((resolve, reject) => {
            const ctx: SP.ClientContext = SP.ClientContext.get_current();
            const parentObj = (caType === CustomActionItem.Web)
                ? ctx.get_web()
                : ctx.get_site();
            ...
            }, this.getErrorResolver(constants.ERROR_MESSAGE_SETTING_CUSTOM_ACTION));
        });
    }
}
